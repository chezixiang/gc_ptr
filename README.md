...
`std` 命名空间特化（`std::hash`、`std::owner_less`、`std::atomic`）已在以下环境验证：

- GCC 13+ / libstdc++（mingw-w64）
- 建议在 Clang / libc++ 和 MSVC / MS STL 上进行额外验证后再用于多编译器项目
...
如果需要非线程安全模式，修改 Makefile 中的 `CXXFLAGS` 移除 `-DGPTR_THREAD`。
